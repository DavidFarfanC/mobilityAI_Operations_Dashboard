...
export const MetroIntegrationExample = () => {
  ...
  const { data: recentIncidents, isLoading: loadingIncidents } =
    useRecentFallDetections()
    ...
    <div className="p-6 space-y-6">
      ...
      <section className="bg-white p-4 rounded-lg shadow">
        ...
        {loadingStatus ? (
          ...
          <div className="space-y-2">
            <p>
              <strong>Línea:</strong> {lineStatus.line_name}
            </p>
            <p>
              <strong>Ruta:</strong> {lineStatus.route}
            </p>
            <p>
              <strong>Saturación:</strong> {lineStatus.saturation}
            </p>
            <p>
              <strong>Trenes activos:</strong> {lineStatus.active_trains.length}
            </p>
            <p>
              <strong>Última actualización:</strong>{' '}
              {new Date(lineStatus.last_updated).toLocaleString()}
            </p>
              ...
              <div className="space-y-2">
                {lineStatus.active_trains.map((train) => (
                  <div key={train.train_id} className="border p-2 rounded">
                    ...
                    <p className="text-sm">
                      Dirección: {train.direction} | Progreso:{' '}
                      {(train.progress_to_next * 100).toFixed(0)}%
                    </p>
                    <p className="text-sm">
                      Vagones: {train.wagons} | Pasajeros:{' '}
                      {train.passengers_per_wagon.join(', ')}
                    </p>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {stations?.slice(0, 6).map((station) => (
              <div key={station.id} className="border p-3 rounded">
                ...
                <p className="text-sm">
                  Personas esperando: {station.people_waiting}
                </p>
                <p className="text-sm">
                  Próximo tren: {station.next_train_arrival} min
                </p>
                {station.has_incident && (
                  <p className="text-red-600 text-sm">
                    ⚠️ {station.incident_message}
                  </p>
                )}
              </div>
            ))}
          ...
          <div className="space-y-3">
            {recentIncidents.map((incident) => (
              <div key={incident.id} className="border p-3 rounded flex gap-3">
                ...
                <div>
                  <p>
                    <strong>Estación:</strong> {incident.station}
                  </p>
                  <p>
                    <strong>Objeto:</strong> {incident.detected_object}
                  </p>
                  <p>
                    <strong>Fecha:</strong>{' '}
      ...
      <section className="bg-blue-50 p-4 rounded-lg">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm">
          ...
          <li>
            Usa los hooks <code>useLineStatus()</code>,{' '}
            <code>useStations()</code>, etc.
          </li>
          <li>
            Para reportar incidentes usa <code>createFallDetection()</code>
          </li>
          <li>
            Todos los servicios están en <code>src/services/</code>
          </li>
          <li>
            Los tipos están definidos en <code>src/types.ts</code>
          </li>
        </ul>
      </section>
    </div>
